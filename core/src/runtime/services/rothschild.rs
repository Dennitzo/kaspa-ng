...
cfg_if! {
    if #[cfg(not(target_arch = "wasm32"))] {
        ...
        enum RothschildEvents {
            SetEnabled { enabled: bool, settings: RothschildSettings },
            UpdateSettings(RothschildSettings),
            Exit,
        }
        ...
        impl RothschildService {
            ...
            pub fn enable(&self, enabled: bool, settings: &RothschildSettings) {
                self.service_events
            ...
            pub fn update_settings(&self, settings: &RothschildSettings) {
                self.service_events
                    .try_send(RothschildEvents::UpdateSettings(settings.clone()))
                    .unwrap();
            }
                    ...
                    loop {
                        select! {
                            msg = this.service_events.receiver.recv().fuse() => {
                                match msg {
                                    Ok(RothschildEvents::SetEnabled { enabled, settings }) => {
                                    ...
                                    Ok(RothschildEvents::UpdateSettings(settings)) => {
                                        *this.settings.lock().unwrap() = settings;
                                        if this.is_enabled.load(Ordering::SeqCst) {
                                            let _ = this.stop_rothschild().await;
                                            let _ = this.start_rothschild().await;
        ...
        impl RothschildService {
            ...
            pub fn enable(&self, _enabled: bool, _settings: &RothschildSettings) {}

            pub fn update_settings(&self, _settings: &RothschildSettings) {}
        }
