...
cfg_if! {
    if #[cfg(not(target_arch = "wasm32"))] {
        ...
        #[derive(Debug, Clone)]
        enum MinerEvents {
            SetEnabled { enabled: bool, settings: CpuMinerSettings },
            UpdateSettings(CpuMinerSettings),
            Exit,
        }
        ...
        impl CpuMinerService {
            ...
            pub fn enable(&self, enabled: bool, settings: &CpuMinerSettings) {
                self.service_events
            ...
            pub fn update_settings(&self, settings: &CpuMinerSettings) {
                self.service_events
                    .try_send(MinerEvents::UpdateSettings(settings.clone()))
                    .unwrap();
            }
                    ...
                    loop {
                        select! {
                            msg = this.service_events.receiver.recv().fuse() => {
                                match msg {
                                    Ok(MinerEvents::SetEnabled { enabled, settings }) => {
                                    ...
                                    Ok(MinerEvents::UpdateSettings(settings)) => {
                                        *this.settings.lock().unwrap() = settings;
                                        if this.is_enabled.load(Ordering::SeqCst) {
                                            let _ = this.stop_miner().await;
                                            let _ = this.start_miner().await;
        ...
        impl CpuMinerService {
            ...
            pub fn enable(&self, _enabled: bool, _settings: &CpuMinerSettings) {}

            pub fn update_settings(&self, _settings: &CpuMinerSettings) {}
        }
