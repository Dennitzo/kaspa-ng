...
async fn main() {
    ...
    let private_key_hex = match args.private_key {
        Some(private_key_hex) => private_key_hex,
        None => {
            warn!("Private key is required. Generate one in Kaspa-NG Wallet (Rothschild Wallet) and rerun with --private-key.");
            std::process::exit(1);
        }
    };
    ...
    let schnorr_key = Keypair::from_seckey_slice(secp256k1::SECP256K1, &private_key_bytes).unwrap();
