...
export default function TransactionDetails({ loaderData }: Route.ComponentProps) {
  ...
  const inputGraphItems = isCoinbase
    ? [{ address: "COINBASE", amount: 0, isCoinbase: true }]
    : isDetailed
      ? allInputItems
              ...
              {inputGraphItems.map((input, index) => {
                ...
                return (
                  <div
                    ...
                    onMouseLeave={clearFlowHover}
                  >
                    <span className="mr-2 w-16 text-left text-xs text-gray-500">
                      {input.isCoinbase
                        ? "COINBASE"
              ...
              <div className="pointer-events-auto absolute" style={{ left: "50%", top: `${(hubY / flowHeight) * 100}%` }}>
                <div
                  ...
                  onMouseEnter={(event) =>
                    handleFlowHover(
                      event,
                      `Total output: ${displayKAS(transactionSum)} KAS`,
                      "wall"
                    )
                  }
                  onMouseLeave={clearFlowHover}
