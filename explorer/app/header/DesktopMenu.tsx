...
const MenuItem = (props: MenuItemProps) => {
  ...
  const InnerLink = () => (
    <div className="group relative p-2 text-black hover:cursor-pointer hover:text-[#70C7BA]">
      <span className="flex flex-row items-center justify-center">
        {props.name}
        {props.expandable && (
          <ChevronUp className="ms-1 h-4 w-4 -rotate-180 duration-200 group-hover:block group-hover:rotate-0" />
        )}
      </span>
          ...
          <div className="absolute left-1/2 z-2 mt-4 hidden w-48 -translate-x-1/2 flex-col rounded-2xl bg-white p-4 text-black shadow-[0px_4px_32px_0px] shadow-[#00000012] group-hover:flex">
            {props.children.map((menuItem) => (
              <NavLink to={menuItem.linkTo} onClick={() => setMenuEnabled(false)}>
                <div className="hover:bg-gray-25 outline-gray-25 transition-ease-out w-full rounded-lg p-2 text-black transition-all duration-300 hover:text-[#70C7BA] hover:outline">
                  {menuItem.name}
                </div>
              </NavLink>
            ))}
