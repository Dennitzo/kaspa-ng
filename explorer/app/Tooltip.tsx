    ...
    <div className={`relative inline-block ${props.className || ""}`}>
      ...
      {props.message && showElement && (
        <>
          ...
          <div
            ref={tooltipDiv}
            className={`fadeIn absolute top-0 left-1/2 z-900 inline-block max-w-[96vw] -translate-x-1/2 -translate-y-[calc(100%+8px)] overflow-hidden rounded-xl bg-black p-2 text-white opacity-100 transition-opacity duration-500 ${props.multiLine ? "whitespace-pre-line break-words" : "text-nowrap"} ${props.contentClassName || ""}`}
          >
            {props.message}
          </div>
        </>
